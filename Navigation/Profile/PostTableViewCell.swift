...
class PostTableViewCell: UITableViewCell {
    ...
    func setData(post: Post) {
        ...
        postImage.image = updateFilter(imageName: post.image)
    }
    ...
    private func updateFilter(imageName: String) -> UIImage? {
        
        let imageProc = ImageProcessor()
        let image = UIImage(named: imageName)
        var newImage: UIImage? = UIImage()
        imageProc.processImage(sourceImage: image!, filter: .motionBlur(radius: 10)) { newImage_ in
            newImage = newImage_
        }
        return newImage
    }
